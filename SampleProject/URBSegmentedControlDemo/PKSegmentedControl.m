...
//  Copyright (c) 2013年 Urban10 Interactive. All rights reserved.
//

#import "PKSegmentedControl.h"
#import <QuartzCore/QuartzCore.h>

@interface UIColor (PKSegmentedControl)
- (UIColor *)adjustBrightness:(CGFloat)amount;
@end

@interface PKSegmentView : UIButton
@property (nonatomic, assign) PKSegmentViewLayout viewLayout;
...
- (void)layoutSegments;
- (void)handleSelect:(PKSegmentView *)segmentView;
- (NSInteger)firstSegmentIndexNearIndex:(NSUInteger)index enabled:(BOOL)enabled;
- (UIImage *)defaultBackgroundImage;
@end

static CGSize const kPKDefaultSize = {300.0f, 44.0f};
...
- (void)initInternal{
    ...
    self.frame = CGRectMake(10.0, 0.0, kPKDefaultSize.width, kPKDefaultSize.height);
    ...
    // layout
    self.layoutOrientation = PKSegmentedControlOrientationHorizontal;
    self.segmentViewLayout = PKSegmentViewLayoutDefault;
...
- (void)insertSegmentWithTitle:(NSString *)title image:(UIImage *)image atIndex:(NSUInteger)segment animated:(BOOL)animated {
	PKSegmentView *segmentView = [PKSegmentView new];
...
- (void)setSegmentBackgroundColor:(UIColor *)segmentBackgroundColor atIndex:(NSUInteger)segment {
	PKSegmentView *segmentView = [self segmentAtIndex:segment];
	if (segmentView) {
...
- (void)setContentEdgeInsets:(UIEdgeInsets)contentEdgeInsets {
	...
	[self.items enumerateObjectsUsingBlock:^(PKSegmentView *segmentView, NSUInteger idx, BOOL *stop) {
...
- (void)setSelectedSegmentIndex:(NSInteger)selectedSegmentIndex {
	if (_selectedSegmentIndex != selectedSegmentIndex) {
		...
		if (_selectedSegmentIndex >= 0)
			((PKSegmentView *)[self segmentAtIndex:_selectedSegmentIndex]).selected = NO;
...
- (void)setSegmentViewLayout:(PKSegmentViewLayout)segmentViewLayout {
...
- (PKSegmentView *)segmentAtIndex:(NSUInteger)index {
...
- (NSInteger)firstSegmentIndexNearIndex:(NSUInteger)index enabled:(BOOL)enabled {
	for (int i = index; i < self.items.count; i++) {
		if (((PKSegmentView *)[self.items objectAtIndex:i]).enabled == enabled) {
			return i;
		}
	}
...
- (void)layoutSegments {
	...
	CGSize segmentSize = (self.layoutOrientation == PKSegmentedControlOrientationVertical) ? CGSizeMake(totalSize.width, totalSize.height / self.items.count) : CGSizeMake(totalSize.width / self.items.count, totalSize.height);
	
	[self.items enumerateObjectsUsingBlock:^(PKSegmentView *item, NSUInteger idx, BOOL *stop) {
		if (self.layoutOrientation == PKSegmentedControlOrientationVertical) {
...
#pragma mark - PKSegmentView

@implementation PKSegmentView {
...
+ (void)initialize {
	...
	PKSegmentView *appearance = [self appearance];
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.viewLayout = PKSegmentViewLayoutDefault;
...
- (void)layoutSubviews {
	...
	if (self.titleLabel.text.length > 0 && self.imageView.image) {
		if (self.viewLayout == PKSegmentViewLayoutVertical) {
...
- (void)setViewLayout:(PKSegmentViewLayout)viewLayout {
	if (viewLayout != _viewLayout) {
		_viewLayout = viewLayout;
		
		if (viewLayout == PKSegmentViewLayoutVertical) {
...
#pragma mark - UIImage+PKSegmentedControl

@implementation UIImage (PKSegmentedControl)
...
#pragma mark - UIColor+PKSegmentedControl

@implementation UIColor (PKSegmentedControl)
